class ZonesController < ApplicationController
  before_filter :authenticate_user!, only: [:create, :update, :destroy]

  # GET /zones
  # GET /zones.json
  def index
    @zones = Zone.order("created_at asc")

    respond_to do |format|
      format.html # index.html.erb
      format.json { render json: @zones }
      format.js { render 'index' }
    end
  end

  # GET /zones/1
  # GET /zones/1.json
  def show
    @zone = Zone.find(params[:id])

    respond_to do |format|
      format.html # show.html.erb
      format.json { render json: @zone }
    end
  end

  # GET /zones/new
  # GET /zones/new.json
  def new
    @zone = Zone.new
    @settings = SettingUtility.settings

    respond_to do |format|
      format.html # new.html.erb
      format.json { render json: @zone }
    end
  end

  # GET /zones/1/edit
  def edit
    @zone = Zone.find(params[:id])
    @settings = SettingUtility.settings

  end

  # POST /zones
  # POST /zones.json
  def create
    @zone = Zone.new(params[:zone])
    #@zone.extract_dimensions

    respond_to do |format|
      if @zone.save
        format.html { redirect_to zones_path, notice: 'Zone was successfully created.' }
        format.json { render json: @zone, status: :created, location: @zone }
        format.js {
          @new_url = edit_zone_path(@zone)
          render 'create' }
      else
        format.html { render action: "new" }
        format.json { render json: @zone.errors, status: :unprocessable_entity }
      end
    end
  end

  # PUT /zones/1
  # PUT /zones/1.json
  def update
    @zone = Zone.find(params[:id])

    if params[:move]
      @zone.position = @zone.position + (params[:move] == 'up' ? -1 : 1)
      @zone.save
    end

    respond_to do |format|
      if @zone.update_attributes(params[:zone])
        format.html { redirect_to zones_path, notice: 'Zone was successfully updated.' }
        format.json { head :no_content }
        format.js {
          @zones = Zone.order('position asc')

          render 'index'}
      else
        format.html { render action: "edit" }
        format.json { render json: @zone.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /zones/1
  # DELETE /zones/1.json
  def destroy
    @zone = Zone.find(params[:id])
    @zone.destroy

    respond_to do |format|
      format.html { redirect_to zones_url }
      format.json { head :no_content }
      format.js {
        @zones = Zone.order('position asc')
        render 'index'
      }
    end
  end


end
